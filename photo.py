...
def show():
    ...
    if raw_images:
        ...
        if st.session_state.show_order:
            st.caption("순서를 바꾸고 싶으면 번호를 수정하세요.")
            for i, (name, img) in enumerate(raw_images):
                col1, col2 = st.columns([1, 4])
                with col1:
                    order = st.number_input(
                        "순서", min_value=1, max_value=len(raw_images),
                        value=st.session_state.order_values.get(i, i + 1),
                        key=f"order_{i}", label_visibility="collapsed"
                    )
    ...
    if images and meeting_name.strip():
        ...
        for page_idx, page_images in enumerate(pages):
            ...
            if page_idx == 0:
                ...
                <div style="text-align:center; margin-bottom:6px;">
                    <span style="font-size:22px; font-weight:700; color:#1a1a2e; font-family:'Malgun Gothic',sans-serif;">{meeting_name.strip()}</span>
                </div>
        ...
        components.html(f"<html><body style='margin:0;padding:0;background:#f5f5f5;'>{html_pages}</body></html>",
                        height=min(len(pages) * (320 * PHOTOS_PER_PAGE + 200), 1200), scrolling=True)
